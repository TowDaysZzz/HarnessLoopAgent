...
export default function App() {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 20; attempt++) {
        ...
        if (current.status === 'indexed') {
          setNotice('笔记已保存并完成检索索引。')
          return
        }
        if (current.status === 'index_failed' || current.last_error) {
          setError(`笔记已保存，但建立索引失败：${current.last_error || '未知错误'}`)
          return
        }
        await new Promise(resolve => window.setTimeout(resolve, 500))
      }
      setNotice('笔记已保存，检索索引仍在后台处理中。')
      await loadNotes()
    } catch (err) {
  ...
  const noteStatus = (status: string) => ({indexed:'可检索',indexing:'索引中',index_failed:'索引失败',delete_pending:'删除中'}[status] || status)
  ...
  return <main className="app-shell">
    ...
    {notice && <div className="notice toast"><span>{notice}</span><button type="button" className="icon" aria-label="关闭提示" onClick={() => setNotice('')}><X size={15}/></button></div>}
  </main>
}
